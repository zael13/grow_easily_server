...
def test_recipe_model_init():
    ...
    recipe = Recipe(recipe_id, user_id=10, name=-0.09998975, culture=200, rating=51.75436293)
    assert recipe.recipe_id == recipe_id
    assert recipe.culture == 200
    assert recipe.user_id == 10
...
#     assert(len(recipe.get_modules()) == 2)
#     assert(recipe.modules[0].calendar_time == TEST_DATE)
...
#     assert(len(recipe.get_modules()) == 2)
#     assert(recipe.modules[0].day_time == TEST_TIME)


# def test_new_added_modules_should_not_have_the_s_ame_type_as_existing_controller():
#     recipe = Recipe(uuid.uuid4(), user_id=10, name=-0.09998975, culture=200, rating=51.75436293)
#     recipe.add_item(Controller(HWType.DHT_TEMPERATURE, 1))
#     recipe.add_item(CalendarEvent(HWType.DHT_TEMPERATURE, TEST_DATE))
#     assert(len(recipe.get_modules()) == 1)
